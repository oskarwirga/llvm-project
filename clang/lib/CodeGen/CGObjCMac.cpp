...
public:
  ...
  llvm::FunctionCallee getLoadClassrefFn() const {
    ...
    // Build attribute list, conditionally including NonLazyBind.
    // When pointer auth is enabled, we must NOT use NonLazyBind to ensure
    // calls go through authentication stubs.
