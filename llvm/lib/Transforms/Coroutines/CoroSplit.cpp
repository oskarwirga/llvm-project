...
void coro::AnyRetconABI::splitCoroutine(Function &F, coro::Shape &Shape,
  ...
  for (auto [Idx, CS] : llvm::enumerate(Shape.CoroSuspends)) {
    ...
    if (!ReturnBB) {
      ...
      // On arm64e, sign the resume function pointer if ptrauth metadata
      // was attached to the coro.id intrinsic by Swift IRGen.
      if (auto *CoroId = Shape.CoroBegin->getId()) {
        if (auto *MD = CoroId->getMetadata("ptrauth.resume")) {
