...
entry:
  ...
  %id = call token (i32, i32, ptr, ptr, ptr, ptr, ...) @llvm.coro.id.retcon.once(i32 8, i32 8, ptr %buffer, ptr @prototype, ptr @allocate, ptr @deallocate), !ptrauth.resume !0
...
; Verify that the ramp function signs the resume pointer.
; CHECK-LABEL: define { ptr, ptr } @test_ptrauth_resume(
; The resume function pointer should be signed with ptrauth.sign
; using key 0 (IA) and a blended discriminator (address-diversified).
; CHECK: call i64 @llvm.ptrauth.blend(i64 %{{.*}}, i64 3909)
; CHECK: call i64 @llvm.ptrauth.sign(i64 ptrtoint (ptr @test_ptrauth_resume.resume.0 to i64), i32 0, i64 %{{.*}})
